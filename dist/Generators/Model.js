...
var ModelsGenerator = (function () {
    function ModelsGenerator(model) {
        ...
        this.base = function (model) {
            return {
                rei: common_tags_1.codeBlock(templateObject_1 || (templateObject_1 = __makeTemplateObject(["\n        type t = {\n          ", "\n        };\n      "], ["\n        type t = {\n          ", "\n        };\n      "])), model.fields.map(helpers_1.toObjectType).join(',\n')),
                re: common_tags_1.codeBlock(templateObject_2 || (templateObject_2 = __makeTemplateObject(["\n        type t = {\n          ", "\n        };\n      "], ["\n        type t = {\n          ", "\n        };\n      "])), model.fields.map(helpers_1.toObjectType).join(',\n'))
            };
        };
        this.create = function (model) {
            var hasRelations = model.fields.some(function (field) { return field.kind === 'object'; });
            return {
                rei: common_tags_1.codeBlock(templateObject_3 || (templateObject_3 = __makeTemplateObject(["\n        let create: (\n          prismaClient,\n          ~select: Select.t=?,\n          ", "\n          ~data: CreateInput.t,\n          unit\n        ) => Js.Promise2.t<t>;\n      "], ["\n        let create: (\n          prismaClient,\n          ~select: Select.t=?,\n          ", "\n          ~data: CreateInput.t,\n          unit\n        ) => Js.Promise2.t<t>;\n      "])), hasRelations ? '~include_: Include.t=?,' : ''),
                re: common_tags_1.codeBlock(templateObject_4 || (templateObject_4 = __makeTemplateObject(["\n        let create = (\n          prismaClient,\n          ~select=?,\n          ", "\n          ~data,\n          ()\n        ) => {\n          Externals.", ".Create.make(\n            prismaClient,\n            {\n              select: select,\n              ", "\n              data: data,\n            }\n          );\n        };\n      "], ["\n        let create = (\n          prismaClient,\n          ~select=?,\n          ", "\n          ~data,\n          ()\n        ) => {\n          Externals.", ".Create.make(\n            prismaClient,\n            {\n              select: select,\n              ", "\n              data: data,\n            }\n          );\n        };\n      "])), hasRelations ? '~include_=?,' : '', model.name, hasRelations ? 'include_: include_,' : '')
            };
        };
        this.findMany = function (model) {
            var hasRelations = model.fields.some(function (field) { return field.kind === 'object'; });
            return {
                rei: common_tags_1.codeBlock(templateObject_5 || (templateObject_5 = __makeTemplateObject(["\n        let findMany: (\n          prismaClient,\n          ~select: Select.t=?,\n          ", "\n          ~where: WhereInput.t=?,\n          ~skip: int=?,\n          ~after: WhereUniqueInput.t=?,\n          ~before: WhereUniqueInput.t=?,\n          ~first: int=?,\n          ~last: int=?,\n          unit\n        ) => Js.Promise2.t<array<t>>;\n      "], ["\n        let findMany: (\n          prismaClient,\n          ~select: Select.t=?,\n          ", "\n          ~where: WhereInput.t=?,\n          ~skip: int=?,\n          ~after: WhereUniqueInput.t=?,\n          ~before: WhereUniqueInput.t=?,\n          ~first: int=?,\n          ~last: int=?,\n          unit\n        ) => Js.Promise2.t<array<t>>;\n      "])), hasRelations ? '~include_: Include.t=?,' : ''),
                re: common_tags_1.codeBlock(templateObject_6 || (templateObject_6 = __makeTemplateObject(["\n        let findMany = (\n          prismaClient,\n          ~select=?,\n          ", "\n          ~where=?,\n          // ~orderBy=?,\n          ~skip=?,\n          ~after=?,\n          ~before=?,\n          ~first=?,\n          ~last=?,\n          ()\n        ) => {\n          Externals.", ".FindMany.make(\n            prismaClient,\n            {\n              select: select,\n              ", "\n              where: where,\n              // orderBy: orderBy,\n              skip: skip,\n              after: after,\n              before: before,\n              first: first,\n              last: last,\n            }\n          );\n        };\n      "], ["\n        let findMany = (\n          prismaClient,\n          ~select=?,\n          ", "\n          ~where=?,\n          // ~orderBy=?,\n          ~skip=?,\n          ~after=?,\n          ~before=?,\n          ~first=?,\n          ~last=?,\n          ()\n        ) => {\n          Externals.", ".FindMany.make(\n            prismaClient,\n            {\n              select: select,\n              ", "\n              where: where,\n              // orderBy: orderBy,\n              skip: skip,\n              after: after,\n              before: before,\n              first: first,\n              last: last,\n            }\n          );\n        };\n      "])), hasRelations ? '~include_=?,' : '', model.name, hasRelations ? 'include_: include_,' : '')
            };
        };
